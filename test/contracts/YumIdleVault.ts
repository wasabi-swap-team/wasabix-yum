...
let YumIdleVaultFactory: ContractFactory;
let AlUSDFactory: ContractFactory;
let ERC20MockFactory: ContractFactory;
let IdleVaultAdapterMockFactory: ContractFactory;
let TransmuterFactory: ContractFactory;

describe("YumIdleVault", () => {
  let signers: Signer[];

  before(async () => {
    YumIdleVaultFactory = await ethers.getContractFactory("YumIdleVault");
    ...
    IdleVaultAdapterMockFactory = await ethers.getContractFactory(
      "IdleVaultAdapterMock"
    );
  });
  ...
  describe("constructor", async () => {
    ...
    context("when governance is the zero address", () => {
      it("reverts", async () => {
        expect(
          YumIdleVaultFactory.connect(deployer).deploy(
            token.address,
            alUsd.address,
            ZERO_ADDRESS,
  ...
  describe("update Alchemist addys and variables", () => {
    ...
    beforeEach(async () => {
      ...
      alchemist = (await YumIdleVaultFactory.connect(deployer).deploy(
        token.address,
        alUsd.address,
  ...
  describe("vault actions", () => {
    ...
    let alUsd: WaToken;
    let alchemist: YumIdleVault;
    let adapter: IdleVaultAdapterMock;
    let harvestFee = 1000;
    let pctReso = 10000;
    ...
    describe("migrate", () => {
      beforeEach(async () => {
        adapter = (await IdleVaultAdapterMockFactory.connect(deployer).deploy(
          token.address
        )) as IdleVaultAdapterMock;
      ...
      context("when caller is current governance", () => {
        ...
        context("when adapter token mismatches", () => {
          ...
          let invalidAdapter: IdleVaultAdapterMock;

          beforeEach(async () => {
            invalidAdapter = (await IdleVaultAdapterMockFactory.connect(
              deployer
            ).deploy(tokenAddress)) as IdleVaultAdapterMock;
          });
    ...
    describe("flush funds", () => {
      ...
      context("when there is at least one vault to flush to", () => {
        ...
        context("when there are multiple vaults", () => {
          let inactiveAdapter: IdleVaultAdapterMock;
          let activeAdapter: IdleVaultAdapterMock;
          let mintAmount = parseEther("5000");

          beforeEach(async () => {
            inactiveAdapter = (await IdleVaultAdapterMockFactory.connect(
              deployer
            ).deploy(token.address)) as IdleVaultAdapterMock;

            activeAdapter = (await IdleVaultAdapterMockFactory.connect(
              deployer
            ).deploy(token.address)) as IdleVaultAdapterMock;
